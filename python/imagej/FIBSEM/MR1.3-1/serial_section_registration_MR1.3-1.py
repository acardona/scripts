...
paramsSIFT.minOctaveSize = 256 #0 will be updated in a clone
paramsSIFT.maxOctaveSize = 4096 #0 will be updated in a clone
...
ignore_images = set([
	#"Merlin-WEMS_24-05-30_104352_0-0-0.dat",
	#"Merlin-WEMS_24-05-30_104352_0-1-0.dat",
])

replace_images = {
	"Merlin-WEMS_24-05-30_104352_0-0-0.dat": "Merlin-WEMS_24-05-30_104352_0-0-0.tif"
}
...
# Skip sections ...
groupNames = groupNames[74:]
tileGroups = tileGroups[74:]

groupNames[7819] = groupNames[7818]
tileGroups[7819] = tileGroups[7818]
...
def sectionOffsets(index): # index is 0-based   <<< ZERO BASED
  ...
  dx += 2500 # Shift the image to centre
  dy += 2500
  return (dx, dy)
...
#roi = Roi(4000,7200,1000,1300) # Neuropil, not full stack. From ~100 to ~16500
#roi = Roi(5400,6700,1000,1000) # Full stack
#roi = Roi(4000,6700,2000,2000)
roi = Roi(3000,5700,4000,4000)

# crop out slices where sample is not in RoI
#groupNames = groupNames[100:16000]
#tileGroups = tileGroups[100:16000]
...
properties = {
 ...
 'RANSAC_maxEpsilon': 5, # default is 25, for ssTEM 40nm sections cross-section alignment, but FIBSEM at 8nm sections is far thinner
 ...
 'min_blockmatching': 5 # Minimum number of pointmatches before falling back to SIFT
}

# Parameters for blockmatching
params = {
 'scale': 0.25, # 50%
 'meshResolution': 5, # 20x20 = 400 points
 ...
 'searchRadius': 200, # has to account for the montages shifting about ~100 pixels in any direction
...
paramsSIFT.fdSize = 8 # default is 4
paramsSIFT.fdBins = 8 # default is 8
paramsSIFT.maxOctaveSize = 250 #int(max(1024, section_width * params["scale"]))
paramsSIFT.steps = 2
paramsSIFT.minOctaveSize = 125 #int(paramsSIFT.maxOctaveSize / pow(2, paramsSIFT.steps))
paramsSIFT.initialSigma = 1.6 # default 1.6

# Parameters for computing the transformation models
paramsTileConfiguration = {
  "n_adjacent": 7, # minimum of 1; Number of adjacent sections to pair up
  "maxAllowedError": 0, # Saalfeld recommends 0
  "maxPlateauwidth": 200, # Like in TrakEM2
  "maxIterations": 300000, # Saalfeld recommends 1000
  "damp": 1.0, # Saalfeld recommends 1.0, which means no damp
...
# Comment this out, don't set to True!
#align = False

if align:

  #volumeImgMontaged = Views.interval([x0, y0, z0], [x0 + width - 1, ])
  ...
  # To be determined:
  imp.setRoi(Roi(0, 0, 14064, 13392))
