...
# If running in fibserver1:
if srcDir.startswith("/data/raw"):
  for tilePaths in tileGroups:
    for i in xrange(len(tilePaths)):
      if tilePaths[i].startswith("/net/fibserver1"):
        tilePaths[i] = "/data" + tilePaths[i][15:]
...
#matricesSIFT = loadMatrices("matrices.csv-combined", csvDirZ)
