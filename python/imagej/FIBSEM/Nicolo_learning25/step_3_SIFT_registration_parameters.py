...
# Can be None
model_path = os.path.join(tgtDir, "montage-csv/labkit/sample_sections-4.tif.classifier") # from LabKit
...
properties = {
 ...
 'n_threads': numCPUs(),
 'roi': map(lambda v: int(v / params_pixels['interim_scale'] + 0.5), [246, 900, 3754, 1722]), # [x, y, width, height] or None. To extract SIFT features  from.
...
paramsSIFT.maxOctaveSize = int(max(1024, section_width * params_pixels["interim_scale"] / 2)) # effective scale of 0.125 if interim_scale was 0.25
paramsSIFT.steps = 5
